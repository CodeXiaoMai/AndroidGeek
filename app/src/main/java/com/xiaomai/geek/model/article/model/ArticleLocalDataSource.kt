...
class ArticleLocalDataSource(private val context: Application) {
    ...
    fun getArticleResponseFromAssets(): Observable<ArticleResponse> {
        Observable.create(object : ObservableOnSubscribe<ArticleResponse> {
            override fun subscribe(e: ObservableEmitter<ArticleResponse>) {
                e.onNext(ArticleResponse(mutableListOf()))
                e.onComplete()
            }
